...
@Component({
  ...
  templateUrl: "./form-pricedichvu.component.html",
  styleUrl: "./form-pricedichvu.component.scss",
})
export class FormPriceDichvuComponent {
  form!: FormGroup;
  isEditMode = true;
  EditData: any;
  item: any = {};
  ...
  onInsert(values: any) {
    ...
    if (item_type) {
      ...
      this._dataService.post(StringAPI.APIServicePrice, this.item).subscribe(
        (res) => {
          this.ref.close(res || []);
        },
        (error) => {},
  ...
  onEdit(values: any) {
    if (this.EditData && this.EditData.id && values) {
      ...
      this._dataService
        .put(StringAPI.APIServicePrice + "/" + this.EditData.id, this.item)
        .subscribe(
          (res) => {
            this.ref.close(res || []);
          },
