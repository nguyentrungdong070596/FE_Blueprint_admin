...
@Component({
  selector: 'app-form-footer', // Đổi từ app-form thành app-form-footer
  standalone: true,
  imports: [CommonModule, FormsModule, ReactiveFormsModule, ButtonModule],
  templateUrl: './form-footer.component.html', // Đổi tên file template
  styleUrl: './form-footer.component.scss' // Đổi tên file style
})
export class FormFooterComponent { // Đổi tên class từ FormHeaderComponent thành FormFooterComponent
  form!: FormGroup;
  isEditMode = true;
  EditData: any;
  item: any = {};
  ...
  onInsert(values: any) {
    ...
    this._dataService.post(StringAPI.APIFooter, this.item) // Đổi APIHeader thành APIFooter
      .subscribe(
        (res) => {
          this.ref.close(res || []);
        },
  ...
  onEdit(values: any) {
    if (this.EditData && this.EditData.id && values) {
      ...
      this._dataService.put(StringAPI.APIFooter + "/" + this.EditData.id, this.item) // Đổi APIHeader thành APIFooter
        .subscribe(
          (res) => {
            this.ref.close(res || []);
          },
